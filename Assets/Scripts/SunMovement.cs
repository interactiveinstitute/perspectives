...
public class SunMovement : MonoBehaviour {
    ...
    public bool affectLightIntensity = false;
    public AnimationCurve lightIntensityByInclinationCurve;
    public bool affectEnvLightingIntensity = false;
    public AnimationCurve envLightIntensityByInclinationCurve;
	...
	void Update() {
        ...
        {
            float angleBasedIntensity = lightIntensityByInclinationCurve.Evaluate(inclinationAngle);
            light.intensity = angleBasedIntensity;
        }
        if (affectEnvLightingIntensity)
        {
            float angleBasedEnvIntensity = envLightIntensityByInclinationCurve.Evaluate(inclinationAngle);
            //light.intensity = angleBasedEnvIntensity;
            RenderSettings.ambientIntensity = angleBasedEnvIntensity;
        }
        

		transform.localRotation = Quaternion.Euler((float)Altitude , 180 + (float)Azimuth,0);
    ...
    //    //Debug.Log("Update ambience: " + altitude);
    //    if(altitude < -6) {
