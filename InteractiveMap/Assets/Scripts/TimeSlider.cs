...
public class TimeSlider : MonoBehaviour {
    ...
    public RectTransform indicatorTranform;
    ...
    void Update()
    {
        float timeScrollPos = timeScrollTransform.localPosition.x;
        float gridPos = gridTransform.localPosition.x;
        float indicatorPos = indicatorTranform.localPosition.x;
        float posX = gridPos - indicatorPos;
        //Debug.Log("indicatorPos, gridPos, posX: " + indicatorPos + ", " + gridPos + ", " +  posX);
    ...
    void MoveTo(DateTime dateTime)
    {
        ...
        float newGridPos = -(spanProgress ) * imageWidth + indicatorTranform.localPosition.x;
    ...
    public void MouseClick(BaseEventData baseEventData)
    {
        ...
        if (clickDistance < 10f)
        {
            ...
            float xClick = localCursor.x - indicatorTranform.localPosition.x;
            //Debug.Log("xClick: " + xClick);
    ...
    public void IndicatorDrag(BaseEventData baseEventData)
    {
        PointerEventData pointerEvent = baseEventData as PointerEventData;
        //Debug.Log("dragging: " + pointerEvent.dragging.ToString());
        //Debug.Log("drag: " + pointerEvent.pointerDrag.name);
        //Debug.Log("delta: " + pointerEvent.delta.x);
        if (pointerEvent.dragging)
        {
            Vector3 indicatorLocalPos = indicatorTranform.position;
            indicatorLocalPos.x += pointerEvent.delta.x;
            indicatorTranform.position = indicatorLocalPos;
        }
    }
}
